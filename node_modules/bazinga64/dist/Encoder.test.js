...
describe('toBase64', () => {
    it('encodes arrays', () => {
        const encoded = bazinga64.Encoder.toBase64(TestValues_1.helloDecodedArray);
        expect(encoded.asString).toBe(TestValues_1.helloEncodedString);
    });
    it('encodes array buffers', () => {
        ...
        const expected = 'owABARn//wKiAKEAWEDEX0UODINkYDHbu76tv17jYQX9SBtvclYEQ7765c26Xd9g90BBZTorb+oHtgqPUJ2PDKWb/XXcQbDiUEenRWUlAaEAWCDfYPdAQWU6K2/qB7YKj1Cdjwylm/113EGw4lBHp0VlJQ==';
    ...
    it('encodes numbers', () => {
        expect(bazinga64.Encoder.toBase64(TestValues_1.numberDecoded).asString).toBe(TestValues_1.numberEncoded);
    });
    it('encodes strings', () => {
        const encoded = bazinga64.Encoder.toBase64(TestValues_1.helloDecodedString);
        expect(encoded.asString).toBe(TestValues_1.helloEncodedString);
    });
    it('encodes texts', () => {
        const text = 'Man is distinguished, not only by his reason, but by this singular passion from other animals, which is a lust of the mind, that by a perseverance of delight in the continued and indefatigable generation of knowledge, exceeds the short vehemence of any carnal pleasure.';
