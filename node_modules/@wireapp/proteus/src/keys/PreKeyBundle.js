"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.PreKeyBundle = void 0;
const cbor_1 = require("@wireapp/cbor");
const sodium = require("libsodium-wrappers-sumo");
const DecodeError_1 = require("../errors/DecodeError");
const IdentityKey_1 = require("./IdentityKey");
const PreKeyAuth_1 = require("./PreKeyAuth");
const PublicKey_1 = require("./PublicKey");
class PreKeyBundle {
    constructor(publicIdentityKey, preKeyIdOrPreKey, publicKeyOrSignature, signatureOrVersion = null, versionOrNone = 1) {
        this.identity_key = publicIdentityKey;
        if (typeof preKeyIdOrPreKey === 'number') {
            this.prekey_id = preKeyIdOrPreKey;
            this.public_key = publicKeyOrSignature;
            this.signature = signatureOrVersion;
            this.version = versionOrNone;
        }
        else {
            this.prekey_id = preKeyIdOrPreKey.key_id;
            this.public_key = preKeyIdOrPreKey.key_pair.public_key;
            this.signature = null;
            this.version = 1;
        }
    }
    static signed(identityPair, prekey) {
        const ratchetKey = prekey.key_pair.public_key;
        const signature = identityPair.secret_key.sign(ratchetKey.pub_edward);
        return new PreKeyBundle(identityPair.public_key, prekey.key_id, prekey.key_pair.public_key, signature);
    }
    verify() {
        if (!this.signature) {
            return PreKeyAuth_1.PreKeyAuth.UNKNOWN;
        }
        if (this.identity_key.public_key.verify(this.signature, this.public_key.pub_edward)) {
            return PreKeyAuth_1.PreKeyAuth.VALID;
        }
        return PreKeyAuth_1.PreKeyAuth.INVALID;
    }
    serialise() {
        const encoder = new cbor_1.Encoder();
        PreKeyBundle.encode(encoder, this);
        return encoder.get_buffer();
    }
    serialised_json() {
        return {
            id: this.prekey_id,
            key: sodium.to_base64(new Uint8Array(this.serialise()), sodium.base64_variants.ORIGINAL),
        };
    }
    static deserialise(buf) {
        return PreKeyBundle.decode(new cbor_1.Decoder(buf));
    }
    static encode(encoder, preKeyBundle) {
        encoder.object(PreKeyBundle.propertiesLength);
        encoder.u8(0);
        encoder.u8(preKeyBundle.version);
        encoder.u8(1);
        encoder.u16(preKeyBundle.prekey_id);
        encoder.u8(2);
        PublicKey_1.PublicKey.encode(encoder, preKeyBundle.public_key);
        encoder.u8(3);
        IdentityKey_1.IdentityKey.encode(encoder, preKeyBundle.identity_key);
        encoder.u8(4);
        return preKeyBundle.signature ? encoder.bytes(preKeyBundle.signature) : encoder.null();
    }
    static decode(decoder) {
        const propertiesLength = decoder.object();
        if (propertiesLength === PreKeyBundle.propertiesLength) {
            decoder.u8();
            const version = decoder.u8();
            decoder.u8();
            const preKeyId = decoder.u16();
            decoder.u8();
            const publicKey = PublicKey_1.PublicKey.decode(decoder);
            decoder.u8();
            const identityKey = IdentityKey_1.IdentityKey.decode(decoder);
            decoder.u8();
            const signature = decoder.optional(() => new Uint8Array(decoder.bytes()));
            return new PreKeyBundle(identityKey, preKeyId, publicKey, signature, version);
        }
        throw new DecodeError_1.DecodeError(`Unexpected number of properties: "${propertiesLength}"`);
    }
}
exports.PreKeyBundle = PreKeyBundle;
PreKeyBundle.propertiesLength = 5;
//# sourceMappingURL=PreKeyBundle.js.map